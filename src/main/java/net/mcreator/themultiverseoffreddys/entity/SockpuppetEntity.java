...
import net.mcreator.themultiverseoffreddys.entity.renderer.SockpuppetRenderer;
...
public class SockpuppetEntity extends UltimateFnafModModElements.ModElement {
	...
	public void initElements() {
		...
		elements.items.add(
				() -> new SpawnEggItem(entity, -1, -16777216, new Item.Properties().group(F2ItemGroup.tab)).setRegistryName("sockpuppet_spawn_egg"));
	}

	@SubscribeEvent
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		boolean biomeCriteria = false;
		if (new ResourceLocation("ultimate_fnaf_mod:blacktomb_yard").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("ultimate_fnaf_mod:choppys_woods").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("ultimate_fnaf_mod:dusting_fields").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("ultimate_fnaf_mod:lunar_mines").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("ultimate_fnaf_mod:pinwheel_funhouse").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
	...
	public static class CustomEntity extends MonsterEntity {
		...
		protected void dropSpecialItems(DamageSource source, int looting, boolean recentlyHitIn) {
			...
			this.entityDropItem(new ItemStack(ChipFuryItem.block));
		}
